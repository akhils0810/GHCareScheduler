...
def calendar_view():
    try:
        ...
        shifts = Shift.get_all(start_date, start_date + timedelta(days=7))
...
def caregiver_view():
    try:
        ...
        shifts = Shift.get_all(start_date, end_date)
...
def add_shift():
    try:
        ...
        caregiver_id = int(request.form.get('caregiver_id'))
        ...
        existing_caregiver_shifts = Shift.get_by_caregiver(caregiver_id, date, date + timedelta(days=1))
        if existing_caregiver_shifts:
        ...
        existing_shifts = Shift.get_by_type(shift_type, date)
        ...
        # Create new shift
        new_shift = Shift.add(date_str, shift_type, caregiver_id)
...
def add_caregiver():
    try:
        ...
        # Generate new ID
        new_id = max([c['id'] for c in caregivers], default=0) + 1
        ...
        data_manager._save_caregivers(caregivers)
...
def delete_caregiver(caregiver_id):
    try:
        ...
        shifts = data_manager.get_shifts_by_caregiver(caregiver_id)
        if shifts:
